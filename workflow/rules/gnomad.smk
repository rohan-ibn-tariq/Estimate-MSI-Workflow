...
rule annotate_sample_with_gnomad:
    ...
    shell:
        ...
        # Add POPULATION_AF header definition ONCE (before any annotation)
        echo '##INFO=<ID=POPULATION_AF,Number=A,Type=Float,Description="gnomAD population allele frequency">' | \
        bcftools annotate -h - working.vcf.gz -Oz -o working_with_header.vcf.gz 2>> ../../{log}
        
        rm working.vcf.gz working.vcf.gz.csi
        mv working_with_header.vcf.gz working.vcf.gz
